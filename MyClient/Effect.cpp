...
CGameObject* CEffect::CreatePlayerSkillEffect(D3DXVECTOR3 pos, XMFLOAT3 & rot, D3DXVECTOR3 & scale, const wstring & _ClipName, const wstring & _AnimName, ANIMATION_TYPE type, const float & animSpeed, const float & time, D3DXVECTOR3 dirVec, const float& speed,const wstring & tagName, Layer layer,D3DXVECTOR3 color)
{
	...
	pEffectScript->SetType(CEffectScript::EFFECT_TYPE::ECHO);
	pEffectScript->SetColor(color);
...
)
{
	...
	m_pTexture->Initialize(pEffectObj);
	m_pTexture->SetPass(3);
	m_pTexture->SetColorToAdd(D3DXVECTOR3(1.3f, 2.0f, 2.5f));
	pEffectObj->AddComponent(m_pTexture);
